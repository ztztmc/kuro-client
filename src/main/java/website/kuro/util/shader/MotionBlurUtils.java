package website.kuro.util.shader;
...
import website.kuro.mixin.accessor.ShaderGroupAccessor;

public class MotionBlurUtils {

    public static final MotionBlurUtils instance = new MotionBlurUtils();

    private static final ResourceLocation location = new ResourceLocation("kuro/shaders/motion blur/post/motion_blur.json");
    ...
    public ShaderGroup getShader() {
        ...
        if (shaderBlur != getBlurFactor()) {
            ((ShaderGroupAccessor)shader).getListShaders().forEach((shader) -> {
